...
{foreach from=$tag->posts->data.items item=postItem}
	...
	<div class="date"><span class="w">{$postItem->published|date_format2:"%m"|month_format:"%nam"}|{$postItem->published|date_format2:"%e"}</span> <span class="y">{$postItem->published|date_format2:"%Y"}</span></div>
	{$postItem->text}
